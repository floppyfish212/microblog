...
# checks whether app.debug being True
#    create a SMTPHandler
#    set its level so it only reports errors and not warnings
#        informational or debugging messages
#    attach the SMTPHandler to the app.logger object
if not app.debug:
...
if not app.debug:
    ...
    file_handler.setFormatter(logging.Formatter(
    	'%(asctime)s %(levelname)s: %(message)s [in %(pathname)s:%s(lineno)d]'))
